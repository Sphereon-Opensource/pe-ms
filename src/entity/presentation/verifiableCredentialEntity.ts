...
export class VerifiableCredentialEntity {
    ...
    @IsDateString({ strict: true }, { message: 'VerifiableCredential.issuanceDate must be a valid ISO8601 date string or EPOCH'})
    //@ts-ignore
    issuanceDate: string;
    @Column()
    @IsDateString({ strict: true }, { message: 'VerifiableCredential.expirationDate must be a valid ISO8601 date string or EPOCH'})
